...
- **ページ翻訳**（`Ctrl+Shift+Y`） — Chromeの「このページを翻訳」相当。本文をその場で日本語に置き換え、もう一度押すと原文に戻ります（翻訳⇄原文のトグル。再翻訳・復元はキャッシュを使うので瞬時）。
- **タブを別ウィンドウへ分離**（`Ctrl+Shift+D`） — 現在のタブを新しいウィンドウに切り出します（左右に並べて見たいとき用）。
- **通常ブラウザとして振る舞う** — `navigator.webdriver` を隠し、低スペック偽装はYouTubeに限定することで、Cloudflareなどのボットチェックに「自動化ブラウザ」と誤認されにくくしています。
...
| `J` / `K` | 前 / 次のタブ |
| `Ctrl+Shift+Y` | ページ翻訳のトグル（日本語 ⇄ 原文） |
| `Ctrl+Shift+D` | 現在のタブを別ウィンドウへ分離 |
