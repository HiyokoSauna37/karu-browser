...
- **Twitchの広告除去** — Twitchは広告を配信本体と同じHLSストリームに縫い込む（サーバー側広告挿入）ため、ドメイン単位のブロックでは触れません。Karuはプレイリストを見て広告区間を検知し、クリーンなプレイリストを取り直して差し替えます。実配信のミッドロールで検証済み（43回検知・43回差し替え成功、画質は元のまま）。差し替えできなかった場合は消音+被いに切り替えます。`settings.json` の `TwitchAdBlock` でOFFにできます。
